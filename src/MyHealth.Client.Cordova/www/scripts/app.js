            ...
            (function (Application) {
                ...
                (function (HomeAppointments) {
                    ...
                    var CardHomeAppointmentDirective = (function () {
                        function CardHomeAppointmentDirective($ionicPopup, notificationService, configService) {
                            var _this = this;
                            this.restrict = 'E';
                            ...
                            this.link = function (scope) {
                                ...
                                scope.openNotificationPopup = function () {
                                    scope.notification =
                                        new HomeAppointments.Notification(_this.configService.General.DEFAULT_TENANT_ID, scope.homeAppointment.patientId, 'choose');
                    ...
                    app.directive('cardHomeAppointment', function ($ionicPopup, notificationService, configService) {
                        return new CardHomeAppointmentDirective($ionicPopup, notificationService, configService);
                    });
                })(HomeAppointments = Application.HomeAppointments || (Application.HomeAppointments = {}));
            })(Application = Cordova.Application || (Cordova.Application = {}));
        })(Cordova = Client.Cordova || (Client.Cordova = {}));
            ...
            (function (Application) {
                ...
                (function (HomeAppointments) {
                    var Notification = (function () {
                        function Notification(tenantId, patientId, message) {
                ...
                (function (HomeAppointments) {
                    ...
                    var HomeAppointmentService = (function () {
                        function HomeAppointmentService($rootScope, dataService) {
                            ...
                            this.visitedHomeAppointments = new Array();
                            this.patients = new Array();
                            this.homeAppointmentById = [];
                            this.visitedHomeAppointmentById = [];
                            this.patientById = [];
                        ...
                        HomeAppointmentService.prototype.digestVisitedHomeAppointmentData = function (data) {
                            var _this = this;
                            data.forEach(function (homeAppointmentData) {
                                if (_this.visitedHomeAppointmentById[homeAppointmentData.appointmentId]) {
                                    _this.visitedHomeAppointmentById[homeAppointmentData.appointmentId].deserialize(homeAppointmentData);
                                }
                                else {
                                    var visitedHomeAppointment = new HomeAppointments.HomeAppointment().deserialize(homeAppointmentData);
                                    _this.visitedHomeAppointmentById[visitedHomeAppointment.appointmentId] = visitedHomeAppointment;
                                    _this.visitedHomeAppointments.push(visitedHomeAppointment);
                                }
                                if (homeAppointmentData.patient) {
                                    _this.digestPatientData([homeAppointmentData.patient]);
                                    var appoID = homeAppointmentData.appointmentId;
                                    var patID = homeAppointmentData.patient.patientId;
                                    _this.visitedHomeAppointmentById[appoID].patient = _this.patientById[patID];
                                }
                            });
                        };
                        ...
                        HomeAppointmentService.prototype.getDetails = function (id, visited) {
                            if (visited) {
                        ...
                        HomeAppointmentService.prototype.getVisited = function (id) {
                            if (id === void 0) { id = null; }
                            if (id) {
                                if (!this.visitedHomeAppointmentById[id]) {
                                    this.visitedHomeAppointmentById[id] = new HomeAppointments.HomeAppointment();
                                }
                        ...
                        HomeAppointmentService.prototype.update = function (visited) {
                            var _this = this;
                            if (visited) {
                                this.dataService.getHomeAppointments(true).then(function (result) {
                                    _this.digestVisitedHomeAppointmentData(result);
                                    _this.$rootScope.$broadcast('homeAppointmentUpdated');
                                });
                            }
                            else {
                                this.dataService.getHomeAppointments(false).then(function (result) {
                ...
                (function (Shared) {
                    ...
                    var DataService = (function () {
                        ...
                        DataService.prototype.getClient = function () {
                            if (!this.client) {
                                this.client = new WindowsAzure.MobileServiceClient(this.configService.Azure.API_URL, '').withFilter(function (request, next, callback) {
                        ...
                        DataService.prototype.getActiveDoctor = function () {
                            ...
                            return new Promise(function (resolve, reject) {
                                if (!_this.activeDoctor.id) {
                                    if (_this.configService.General.REQUIRE_LOGIN) {
                                        _this.getLoggedDoctor().then(function (doctor) {
                                            resolve(doctor);
                                        });
                                    }
                                    else {
                                        _this.getDefaultDoctor().then(function (doctor) {
                                            resolve(doctor);
                                        });
                                    }
                                }
                        ...
                        DataService.prototype.getLoggedDoctor = function () {
                            var _this = this;
                            return new Promise(function (resolve, reject) {
                                _this.login().then(function () {
                                    var tenantId = _this.configService.General.DEFAULT_TENANT_ID;
                                    _this.getClient().invokeApi('LoggedUser', {
                                        method: 'get',
                                        parameters: { tenantId: tenantId }
                                    }).done(function (response) {
                                        _this.activeDoctor.deserialize(response.result);
                                        _this.configService.General.DEFAULT_DOCTOR_GUID = _this.activeDoctor.id;
                        ...
                        DataService.prototype.getDefaultDoctor = function () {
                            var _this = this;
                            return new Promise(function (resolve, reject) {
                                if (!_this.activeDoctor.id) {
                                    _this.login().then(function () {
                                        var doctorsTable = _this.getClient().getTable('doctor');
                                        var doctorId = _this.configService.General.DEFAULT_DOCTOR_GUID;
                                        var tenantId = _this.configService.General.DEFAULT_TENANT_ID;
                                        doctorsTable.where({
                                            id: doctorId,
                                            tenantId: tenantId
                        ...
                        DataService.prototype.getHomeAppointments = function (visited) {
                            var _this = this;
                            return new Promise(function (resolve, reject) {
                                _this.getActiveDoctor().then(function (doctor) {
                                    ...
                                    homeAppointmentsTable.take(10)
                                        ...
                                        tenantId: _this.configService.General.DEFAULT_TENANT_ID,
                                        visited: visited
                                    })
                                        .read()
